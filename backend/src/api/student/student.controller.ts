...
export class StudentController {
  ...
  @ApiResponse({ status: 200, description: 'Ranking de ofensivas retornado com sucesso.' })
  async getStreakLeaderboard(@Request() req: { user: { id: string } }) {
    return this.getStudentStreakLeaderboardUseCase.execute({ userId: req.user.id });
  }
}
