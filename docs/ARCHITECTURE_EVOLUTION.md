...
Embora organizado sob a estrutura de modulos do NestJS, o codigo original persistia com logicas misturadas no escopo do framework. Como exercício final de maturidade, o projeto abracou a **Arquitetura Limpa** unida ao **Domain-Driven Design**.
...
### 6. Refinamento de Responsabilidades e Regras de Negócio (BDD)
Recentemente, a arquitetura passou por um refinamento fino focado em Engenharia de Requisitos e UX/DX:
- **Separação de Módulos (Account vs Student):** O módulo `student` estava concentrando lógicas de gestão de conta (alterar senha, mudar nome). Isso foi quebrado: `account` agora lida com o autoatendimento (aplicável a admins também) e `student` foca estritamente na jornada gamificada do aluno (XP, leaderboard, dashboard).
- **Mapeamento de Regras em BDD:** As regras do sistema saíram da abstração do código e foram documentadas no arquivo `BUSINESS_RULES.md` (agora em `docs/BUSINESS_RULES.md`) usando linguagem de produto (Dado-Quando-Então). Isso serve como contrato estrito para testes e como insumo para a banca do TCC.
- **Validações na Camada de Aplicação:** A remoção de exceções de domínio inúteis (código morto) e a implementação correta de erros como `InvalidReorderError` e tratamento global de erros do Prisma (`P2002`) provaram a resiliência do sistema aos piores cenários (falhas de concorrência ou dados malformados na reordenação).
