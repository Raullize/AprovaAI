...
export default function Leaderboard() {
  ...
  const [streakLeaderboardData, setStreakLeaderboardData] =
    useState<StreakLeaderboardResponse>({
      topUsers: [],
      currentUserRank: -1,
      currentUserEntry: null,
    });
  ...
  const streakRows: LeaderboardRowData[] = streakLeaderboardData.topUsers.map(
    (item) => ({
      rank: item.rank,
      fullName: item.fullName,
      username: item.username,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Veja os estudantes que estão se destacando em XP e Ofensivas na
            plataforma!
          </p>
        </div>
      </div>
            ...
            <>
              {activeTab === 'xp' ? (
                ...
                <div className="space-y-4">
                  ...
                  {!isCurrentUserInStreakTop &&
                    streakLeaderboardData.currentUserEntry &&
                    streakLeaderboardData.currentUserRank > 0 && (
                      <div className="mt-4 pt-4 border-t border-dashed border-slate-200">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-2 px-3">
                          Sua posição
                        </p>
