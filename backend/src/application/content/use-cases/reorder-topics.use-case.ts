...
> {
  ...
  async execute(request: ReorderTopicsRequest): Promise<void> {
    ...
    if (!referenceTopic) {
      throw new InvalidReorderError(`O tópico com ID ${request.ids[0]} não foi encontrado.`);
    }

    const allTopicsInScope = await this.topicRepository.findByExamId(referenceTopic.examId);
    const scopeIds = allTopicsInScope.map((t) => t.id);

    if (request.ids.length !== scopeIds.length) {
      throw new InvalidReorderError('A lista de reordenação deve conter exatamente todos os tópicos do exame atual.');
    }

    const isValid = request.ids.every((id) => scopeIds.includes(id));
    if (!isValid) {
      throw new InvalidReorderError('Um ou mais IDs informados não pertencem a este exame ou são inválidos.');
    }
