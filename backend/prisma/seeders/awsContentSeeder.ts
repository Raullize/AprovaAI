...
export async function seedAwsContent(prisma: PrismaClient) {
  ...
  const topic1 = await prisma.topic.upsert({
    ...
    update: {},
    create: {
      id: buildSeedUuid('topic:aws-cpp:introducao-a-cloud'),
  ...
  const topic2 = await prisma.topic.upsert({
    ...
    update: {},
    create: {
      id: buildSeedUuid('topic:aws-cpp:seguranca-e-conformidade'),
  ...
  if (demoUser) {
    ...
    if (lvl1Questions.length >= 10) {
      ...
      const getXpMultiplier = (stars: number) => {
      ...
      const totalDemoXp = [1, 2].reduce(
        (total, stars, index, attempts) => {
          ...
          return (
            total +
            Math.round(
              (getXpMultiplier(stars) - getXpMultiplier(previousBest)) *
                lvl1.xpReward,
            )
          );
        },
