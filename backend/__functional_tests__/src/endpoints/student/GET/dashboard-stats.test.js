const { createPostRequest, createGetRequestWithAuth, createGetRequest } = require("../../../functionalTestHelper");
...
describe("GET /student/dashboard-stats", () => {
    ...
    it("deve retornar estatisticas do painel sem informar o mes", async () => {
        const response = await createGetRequestWithAuth("/student/dashboard-stats", studentToken);
    ...
    it("deve retornar estatisticas do painel informando um mes especifico", async () => {
        const response = await createGetRequestWithAuth("/student/dashboard-stats?month=2026-05", studentToken);
