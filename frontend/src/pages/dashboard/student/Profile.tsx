...
export default function Profile() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'streak' | 'achievements'>('overview');
    ...
    <div className="min-h-screen bg-slate-50/50 px-4 py-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex border-b border-slate-200/80 gap-6">
          ...
          >
            <Zap className={cn('h-4 w-4', activeTab === 'overview' && 'fill-current animate-pulse')} />
            Visão Geral
            {activeTab === 'overview' && (
          ...
          >
            <Flame className={cn('h-4 w-4', activeTab === 'streak' && 'fill-current animate-pulse')} />
            Ofensiva
            {activeTab === 'streak' && (
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600 rounded-t-full" />
            )}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <Card padding="large">
              ...
              {history.length === 0 ? (
                <p className="text-slate-400 text-sm py-4">Você ainda não realizou nenhum simulado.</p>
              ) : (
                <div className="space-y-3">
                  {history.slice(0, 3).map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 bg-white hover:border-slate-200 transition-colors">
                      <div>
                        <p className="font-bold text-slate-700 text-sm">{item.simulation?.name || 'Simulado'}</p>
                        <p className="text-xs text-slate-400 mt-0.5">
                          {new Date(item.createdAt).toLocaleDateString('pt-BR')} • {Math.round((item.timeSpent || 0) / 60)} min
                        </p>
                      </div>
        ...
        {activeTab === 'achievements' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <Card padding="large" className="bg-gradient-to-r from-indigo-600 via-indigo-750 to-violet-700 text-white shadow-xl shadow-indigo-500/10 border-0 overflow-hidden relative">
              ...
              <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-indigo-50 text-sm mt-2 max-w-xl leading-relaxed">
                    Desbloqueie conquistas respondendo questões, completando simulados e mantendo sua ofensiva de estudos ativa!
                  </p>
                </div>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div
                          className={cn(
                            ...
                            item.isUnlocked ? 'scale-100 hover:scale-110' : 'scale-95',
                            item.color
                          )}
                        >
